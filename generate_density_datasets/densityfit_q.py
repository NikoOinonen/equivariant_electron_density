...
elements = [("H", 2), ("C", 3), ("N", 4), ("O", 3), ("F", 2), ("Si", 3), ("P", 4), ("S", 3), ("Cl", 2), ("Br", 2)]

psi4.set_memory('8 GB')
psi4.set_num_threads(16)
psi4.set_options({"reference": "rohf"}) # Need open shell

for element, multiplicity in elements:

    print(f"Element {element}")

    molstr = f"{element} 0.0 0.0 0.0\n symmetry c1 \n no_reorient \n no_com \n"
    mol = psi4.geometry(molstr)
    mol.set_multiplicity(multiplicity)

    print("Computing " + theory + " gradient...")
    psi4.core.clean()
    psi4.core.set_output_file(f'output_{element}.dat', False)
    ...
    f = open(f"{element}_{auxbasis}_density.out", "w+")
    counter = 0
    totalq = 0.0
    newtotalq = 0.0
    for i in range(0, mol.natom()):
