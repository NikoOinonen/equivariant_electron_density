...
def gau2grid_density_parallel(x, y, z, data, ml_y, rs):
    ...
    with futures.ProcessPoolExecutor(max_workers=num_atoms) as pool:
        for total in pool.map(
            get_dens,
            data.pos.cpu().detach().numpy(),
            data.full_c.cpu().detach().numpy(),
...
def compute_potential_field(xs, ys, zs, data, y_ml, Rs, interatomic=False, intermolecular=False, rad=3.0):
    ...
    for x_pt, y_pt, z_pt in zip(xs, ys, zs):
        ...
        if interatomic == True:
            target_ele_potential, target_ele_field, ml_ele_potential, ml_ele_field = get_ele_potential_field(
                position, ints, results, nbf, data, y_ml, Rs, zero_atom=counter
            )
        elif intermolecular == True:
            target_ele_potential, target_ele_field, ml_ele_potential, ml_ele_field = get_ele_potential_field(
                position, ints, results, nbf, data, y_ml, Rs, zero_atom=skip_atoms
            )
        else:
            target_ele_potential, target_ele_field, ml_ele_potential, ml_ele_field = get_ele_potential_field(
                position, ints, results, nbf, data, y_ml, Rs
            )
